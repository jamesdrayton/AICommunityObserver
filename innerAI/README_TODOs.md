...
- Convert each ticket into a detailed issue file (one markdown per ticket) with priority and acceptance tests.
- Create a CSV-export of the tickets (not requested now) for import later.
- Start implementing the lower-risk tickets (e.g., `API-3`, `API-4`, `DOC-1`) and run unit tests.

If you want, I'll mark the detailed-ticket step complete in the tracked TODOs and create individual ticket files. (Currently the detailed-ticket task is in-progress.)
